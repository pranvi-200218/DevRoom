...
function Avatar({ label, size = "w-10 h-10 text-sm" }) {
  return (
    <div
      className={`${size} rounded-full bg-surface-container-highest border-2 border-surface flex items-center justify-center font-bold text-primary shrink-0`}
    >
      {initials(label)}
    </div>
  );
...
export default function ProjectDashboard() {
  ...
  useEffect(() => {
    ...
    if (pctNumRef.current) {
      ...
      gsap.to(obj, {
        ...
        ease: "power2.out",
        onUpdate: () => {
          pctNumRef.current.textContent = Math.round(obj.val);
        },
      });
    }
  ...
  useEffect(() => {
    ...
    activity.forEach((a) => {
      if (!seenActivityIds.current.has(a.$id)) {
        ...
        const el = activityListRef.current.querySelector(
  ...
  function handleHeaderSearchKeyDown(e) {
    ...
    if (memberHit) {
      ...
      toast.show(
        `No matches for "${headerSearch.trim()}" in this project's members or files.`,
        { type: "info" },
      );
    }
  }
    ...
    <>
      ...
      <aside className="gsap-sidebar w-sidebar-width h-full fixed left-0 top-0 bg-surface-dim border-r border-white/5 flex flex-col p-4 z-50">
        ...
        <nav className="flex-1 space-y-1">
          <a
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors bg-surface-container-high text-primary font-medium active:scale-95 duration-100"
            href="#"
          >
            <i className={`${mi("home")} w-[18px] text-center`} />
            <span className="font-body-sm text-body-sm">Home</span>
          </a>
          <a
            onClick={() => navigate("/dashboard")}
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors text-on-surface-variant hover:text-on-surface hover:bg-white/5 active:scale-95 duration-100 cursor-pointer"
          >
            <i className={`${mi("account_tree")} w-[18px] text-center`} />
          ...
          <a
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors text-on-surface-variant hover:text-on-surface hover:bg-white/5 active:scale-95 duration-100 cursor-pointer"
          >
            <i className={`${mi("settings")} w-[18px] text-center`} />
        ...
        <div className="mt-auto pt-6">
          <button
            onClick={() => navigate("/dashboard")}
            className="w-full flex items-center justify-center gap-2 bg-primary text-on-primary py-2.5 rounded-lg font-medium hover:brightness-110 transition-all active:scale-95"
          >
            <i className={`${mi("add")} text-[14px]`} />
      ...
      <header className="gsap-topbar h-16 fixed top-0 right-0 z-40 bg-surface/60 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-gutter ml-sidebar-width w-[calc(100%-260px)]">
        <div className="flex items-center gap-4 bg-surface-container-lowest border border-white/5 rounded-lg px-3 py-1.5 w-96 focus-within:ring-1 focus-within:ring-primary transition-all">
          <i
            className={`${mi("search")} text-on-surface-variant text-[16px]`}
          />
          <input
          ...
          />
          <span className="text-on-surface-variant/30 text-[10px] font-code-sm border border-white/10 rounded px-1">
            ↵
          </span>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => navigate(`/project/${projectId}/settings`)}
            className="h-8 w-8 rounded-full overflow-hidden border border-white/10"
        ...
        <div className="gsap-panel flex-1 overflow-y-auto custom-scrollbar p-gutter space-y-8 max-w-container-max">
          ...
          <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <nav className="flex items-center gap-2 text-on-surface-variant mb-2">
                <span
                  onClick={() => navigate("/dashboard")}
                  className="text-label-caps font-label-caps cursor-pointer hover:text-on-surface"
                >
                  Projects
                </span>
                <i className={`${mi("chevron_right")} text-[11px]`} />
            ...
            <div className="flex items-center -space-x-3">
              ...
              <button
                ...
                className="w-10 h-10 rounded-full border-2 border-dashed border-white/20 bg-surface-container-low flex items-center justify-center text-on-surface-variant hover:border-primary hover:text-primary transition-all active:scale-90"
                title="Manage members"
              >
                <i className={`${mi("add")} text-[16px]`} />
              </button>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 glass-panel-dash p-8 rounded-xl relative overflow-hidden group">
              <div className="relative z-10 flex flex-col h-full justify-between">
                <div>
                  <span className="text-label-caps font-label-caps text-primary border border-primary/20 bg-primary/5 px-2 py-1 rounded">
                    Team Onboarding
                  </span>
                  <h3 className="text-headline-md font-code-sm mt-4">
                ...
                <div className="mt-8">
                  <div className="w-full bg-white/5 h-2 rounded-full overflow-hidden">
                    <div
                      ref={pctBarRef}
                      className="bg-primary h-full primary-glow"
            ...
            <div className="glass-panel-dash p-8 rounded-xl flex flex-col justify-between border-primary/20 primary-glow">
              <div>
                <i className={`${mi("schedule")} text-primary text-[26px]`} />
                <h3 className="font-headline-md text-headline-md mt-4">
                  Last Activity
                </h3>
              </div>
              <div>
                <p className="text-white font-code-sm text-body-lg">
                  Updated {relativeTime(project.$updatedAt)}
                </p>
                <p className="text-on-surface-variant font-code-sm text-[13px] mt-1">
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div
              ...
              className="glass-panel-dash p-6 rounded-xl group hover:border-primary/40 transition-all cursor-pointer"
            >
              ...
              <div className="flex items-center text-primary text-label-caps font-label-caps gap-2 group-hover:translate-x-1 transition-transform">
                ...
                <i className={`${mi("arrow_forward")} text-[13px]`} />
              </div>
            </div>
            <div
              onClick={() => navigate(`/project/${projectId}/resources`)}
              className="glass-panel-dash p-6 rounded-xl group hover:border-secondary/40 transition-all cursor-pointer"
            >
              <div className="w-11 h-11 rounded-full border border-secondary/30 flex items-center justify-center text-secondary mb-6 group-hover:border-secondary group-hover:bg-secondary/5 transition-all">
                <i className={`${mi("folder_managed")} text-[18px]`} />
              </div>
              <h4 className="font-headline-md text-[20px] mb-2">
                Resource Vault
              </h4>
            ...
            <div
              ...
              className="glass-panel-dash p-6 rounded-xl group hover:border-tertiary/40 transition-all cursor-pointer"
            >
              <div className="w-11 h-11 rounded-full border border-tertiary/30 flex items-center justify-center text-tertiary mb-6 group-hover:border-tertiary group-hover:bg-tertiary/5 transition-all">
                <i className={`${mi("forum")} text-[18px]`} />
              </div>
              <h4 className="font-headline-md text-[20px] mb-2">Team Chat</h4>
              <p className="text-on-surface-variant font-body-sm mb-4">
                {messagesLoading
                  ? "Loading messages…"
                  : `${messages.length} message${messages.length === 1 ? "" : "s"} in #general.`}
              </p>
          ...
          <section className="glass-panel-dash rounded-xl p-8">
            ...
            {filesLoading ? (
              ...
              <div className="h-48 flex flex-col items-center justify-center gap-3 text-center">
                <i
                  className={`${mi("folder_off")} text-[32px] text-outline-variant`}
                />
                ...
                {filesByType.map(([label, count]) => (
                  ...
                  >
                    <div className="w-full flex-1 flex items-end">
                      ...
                      >
                        <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-primary font-code-sm text-[11px]">
                          {count}
                        </div>
                      </div>
                    </div>
        ...
        <aside className="w-80 border-l border-white/5 bg-surface-container-lowest overflow-hidden flex flex-col">
          <div className="p-6 border-b border-white/5">
            <h3 className="text-label-caps font-label-caps text-on-surface-variant flex items-center gap-2">
              <span
                className={`w-1.5 h-1.5 rounded-full ${activityLoading ? "bg-secondary animate-pulse" : "bg-primary"}`}
              ></span>{" "}
              Recent Activity
            </h3>
          </div>
          ...
          >
            ...
            {!activityLoading && activity.length === 0 && (
              <div className="text-center py-8">
                <i
                  className={`${mi("history")} text-[26px] text-outline-variant mb-2 block`}
                />
                <p className="text-on-surface-variant text-sm">
              ...
              activity.map((a) => (
                ...
                >
                  <div className="shrink-0 mt-0.5 w-7 h-7 rounded-lg bg-surface-container-highest flex items-center justify-center text-primary">
                    <i className={`${mi(activityIcon(a.type))} text-[13px]`} />
                  </div>
                  <div className="min-w-0">
                    <p className="font-body-sm text-body-sm text-on-surface leading-snug">
                      {describeActivity(a)}
                    </p>
                    <p className="text-[11px] text-on-surface-variant mt-0.5 font-code-sm">
          ...
          >
            <i className={`${mi("forum")} text-[13px]`} />
            {messagesLoading
              ? "Open chat"
              : `Open chat · ${messages.length} message${messages.length === 1 ? "" : "s"}`}
          </button>
        </aside>
      </main>
    </>
