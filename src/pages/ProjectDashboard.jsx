...
export default function ProjectDashboard() {
  ...
  const recentMessages = useMemo(() => [...messages].reverse().slice(0, 6), [messages]);
...
<span className="font-body-sm text-body-sm">New Project</span>
    ...
    <header className="h-16 fixed top-0 right-0 z-40 bg-surface/60 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-gutter ml-sidebar-width w-[calc(100%-260px)]">
        ...
        <div className="flex items-center gap-4">
            ...
            >
<span className="material-symbols-outlined text-on-surface-variant" title="Notifications">notifications</span>
</button>
...
<span className="material-symbols-outlined" data-icon="add">add</span>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 glass-panel-dash p-8 rounded-xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 flex flex-col h-full justify-between">
                        <div>
                            ...
                            <h3 className="text-headline-md font-headline-md mt-4">{onboardingPct}% Active</h3>
                ...
                <div className="glass-panel-dash p-8 rounded-xl flex flex-col justify-between border-primary/20 primary-glow">
                    ...
                    <div>
                        <p className="text-white font-medium text-body-lg">Updated {relativeTime(project.$updatedAt)}</p>
                        <p className="text-on-surface-variant font-body-sm mt-1">Created {relativeTime(project.$createdAt)}</p>
                    </div>
                </div>
            </section>
            {/* Quick Link Cards */}
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div onClick={() => navigate(`/project/${projectId}/chat`)} className="glass-panel-dash p-6 rounded-xl group hover:border-tertiary/40 transition-all cursor-pointer">
                    <div className="w-12 h-12 rounded-lg bg-tertiary/10 flex items-center justify-center text-tertiary mb-6 group-hover:scale-110 transition-transform">
                        <span className="material-symbols-outlined text-[28px]" data-icon="forum">forum</span>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                ...
                {!messagesLoading && recentMessages.length === 0 && (
                  <div className="text-center py-8">
                    <span className="material-symbols-outlined text-[32px] text-outline-variant mb-2 block">chat_bubble_outline</span>
                    <p className="text-on-surface-variant text-sm mb-4">No recent activity yet.</p>
                    <button
                        ...
                        <div className="relative shrink-0">
                            {m.authorAvatar ? (
                              <div className="w-8 h-8 rounded-full overflow-hidden border border-white/10">
                                <img className="w-full h-full object-cover" alt={m.authorName} src={m.authorAvatar} />
                              </div>
                            ) : (
                              <Avatar label={m.authorName} size="w-8 h-8 text-[11px]" />
                            )}
                        </div>
                        <div className="min-w-0">
                            <p className="font-body-sm text-body-sm truncate">
                              <span className="text-white font-medium">{m.authorName || "Someone"}</span>{" "}
                              {m.text}
                            </p>
                            <p className="text-[12px] text-on-surface-variant mt-1">{relativeTime(m.$createdAt)} • #general</p>
                        </div>
                    </div>
                  ))}
            </div>
